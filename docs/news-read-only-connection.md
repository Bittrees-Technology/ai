...
3. **Load my News articles** makes an explicit read. Only `get_connection` and `list_articles` are callable; source instructions or advertised tools cannot select another method. The connection may have more scopes in News, but this companion implements no curation, publication, subscription or sending operation.
...
No task/source adapter, local News summarization, saved memory, curation preview, public publication, delivery or schedule control is supplied by this read-only step. Those remain separate checklist work. Acer-server's model, runtime and news jobs remain independent and unchanged.
...
- [Actual integration receipt](evidence/news-read-only-integration-2026-09-23.json) uses the real News MCP handler at merged `41d7f2c9849d4d09e5b3fd56255154ccee465c24`, disposable PostgreSQL accounts and in-memory credentials. It verifies disjoint private article reads, owner isolation, source expiry/revocation, only the two read tools, and unchanged curation/newspaper/subscription/delivery/processing tables. CI pins that News revision and repeats the check on PostgreSQL17.
