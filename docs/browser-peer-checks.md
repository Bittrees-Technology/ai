...
The retained [browser task-consent provider](browser-task-consent.md) validates completed proof and current key/peer/permission rows inside each guarded outbox transaction. A prior asynchronous `validFor` result must not be cached as authority. The legacy callback-only outbox remains for protocol fixtures and compatibility; task dispatch is not mounted in the signed-in page.
...
Browser common database5 added retained task consent; current database6 adds encrypted task preparations; Mac task23 and remote8 remain unchanged. Installed PR40, prepared PR142, local model defaults and Acer-server's model/runtime/news jobs remain unchanged. No live deployment, personal-profile migration, local browser/native automation or task permission is enabled. Signed delivery, historical restore/rotation, lease renewal, relay retention and independent/personal/native acceptance remain open.
