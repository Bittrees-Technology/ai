...
Leaving the window conceals the peer review without discarding resumable key setup in the other panel. Scope changes, another key/registration action, Escape, another peer action, destruction, clock reversal and wall/monotonic review expiry discard transient inputs. A peer action synchronously closes registration and key reviews before capturing the host cancellation counter; deferred changes in another panel cannot invalidate a fresh peer snapshot. That counter is a view coordination signal only, never authority. Before confirmation the UI checks the original local key snapshot; the backend separately checks atomic current state.
...
Disposable CI tests drive the actual built page with SIWE, HTTPS cookies, PostgreSQL and IndexedDB. They use actual retained Mac lifecycle/peer modules with disposable encrypted SQLite and simulated native slots, never personal Keychain access. Cases cover actual invitation exchange and reload/export, independent comparison, owner/recipient/expiry rejection, focus and key-control loss, review expiry, offline maintenance, cross-account display isolation, same-registration deletion fence, retired-key refusal, logout during verification and a rejected post-commit verification response. Desktop/phone previews are retained separately for visual inspection.
