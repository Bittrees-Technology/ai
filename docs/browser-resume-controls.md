...
Five production-browser scenarios use actual Mac resume consent/offer modules with disposable encrypted storage and synthetic native slots. They cover exact import/review/save, unchanged paused Mac task, retained browser choices after reload, history export, offline revoke/delete, tampered input/selection changes, Escape/focus/cross-panel cancellation, expiry after acknowledgement, and late inspection cancellation across three engines. Existing consent-core tests cover replay, storage atomicity, expiry and identity authority; neither narrow suite substitutes for full browser/native and migration acceptance. No browser test runs on the user's Mac.
