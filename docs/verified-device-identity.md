...
The [signed-in browser recovery page](production-browser-recovery.md) now mounts verified registration and saved-backup controls. Historical endpoint recovery, peer rotation, encrypted transport, scoped resume and independent/personal native acceptance remain open; browser registration and retained keys alone do not activate private access. The existing storage recovery kit does not contain endpoint private keys.
