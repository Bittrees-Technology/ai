...
Tests cover actual SQLite reopen, concurrent handles, one-use expiry, lost acceptance/revocation replies, interrupted native storage, delayed writes after stop/deletion, global deletion during a held response, identity mismatch, metadata replacement/rollback, credential corruption, cleanup retry and backup restore. The CI-only `scripts/check-packaged-private-relay.mjs` additionally exercises the real bundled Keychain helper, a distinct endpoint-key account, an untrusted native reader, reopen, restore locking and cleanup on a disposable Mac runner. This new native acceptance is authored but must pass final-head CI before merge. The full synthetic HTTPS/PostgreSQL integration exercises the real native enrollment bridge, journal, relay client and revocation endpoints. `scripts/check-private-relay-custody-upgrade.mjs` verifies an actual compiled task23 engine against task24 with disposable data; recorded evidence lives under `docs/evidence/`.
