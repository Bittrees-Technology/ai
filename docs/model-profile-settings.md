...
Profile choices in Tasks, CRM, AutoNote, Mail and Templates show settings to distinguish profiles using the same model. Saved profiles also show their IDs. Projections that only include model/ID retain an ID fallback. Source-free run history displays recorded settings from that run, not the current default. Connected-app history concealment remains unchanged.
