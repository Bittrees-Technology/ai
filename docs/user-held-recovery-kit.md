...
## Native byte adapter regression

A clean PR103 archive exposed a mismatch that in-memory test adapters did not: `@napi-rs/keyring` 2.1.0 returned a JavaScript `number[]` on the tested Mac, although its asynchronous declaration specifies `Uint8Array`. Missing entries also returned `null`. Passing the array through the add-only adapter caused recovery's `byteLength` comparison to reject the correct existing key. The rejected archive was not installed.

`macKeychainEntry` now normalizes actual byte arrays/typed bytes into an owned `Uint8Array`, maps null/undefined to missing, and rejects malformed/coerced values. The runtime regression in `scripts/check-packaged-key-recovery.mjs` runs after packaging in Mac CI, using the bundled Node/native addon and KeyInstall helper with a unique disposable `recovery-test-*` credential. It creates a kit, recovers a real synthetic task/memory backup with the matching native credential, preserves a conflicting attempted add, verifies retained-copy protections/deletion and source-bound memory, and checks wrong-code isolation. Fresh missing-entry recovery uses simulated storage; it does not claim new-helper ACL/access-prompt acceptance. The disposable credential is deleted and its absence checked; personal entries are never read.
