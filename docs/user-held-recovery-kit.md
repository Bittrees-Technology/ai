...
This module wraps the existing personal storage key for recovery after losing its Keychain entry. Kit creation has a native development preview, disabled by default; there is no HTTP or public CLI exposure. User-facing key installation is not connected. Independent crypto review and native acceptance remain open. No personal key was exported during implementation.
...
The helper is built into development packages but is not connected to user-facing recovery. The gated setup preview below implements code display/save confirmation. Authenticated recovery input, end-to-end native access prompts, signed delivery and independent crypto review remain open. Helper-created entries may require a later native Keychain access decision; matching-key and add-only tests are not proof of unattended personal-device recovery.
