...
Verification uses transport tests plus the actual React dashboard in disposable GitHub Chromium, Firefox and WebKit with synthetic routed HTTP. It covers stale history success/denial, unavailable-history refresh, locking during profile creation and export, fresh pairing and deletion/remounted controls. This does not establish native window/Keychain acceptance or a real-user source pilot. No database schema, models, defaults, remote service or Acer-server configuration is changed.
