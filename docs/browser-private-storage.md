...
This is repository/CI integration, not a personal-profile migration or live deployment. Mac task schema23, remote database schema8, installed PR40, prepared PR142 and Mac model defaults are unchanged. Acer-server's existing model, runtime and news jobs remain unchanged; Mac inference has no Acer fallback. Production task UI/dispatch/transport, relay retention, independent protocol review and personal browser/native acceptance remain open.
