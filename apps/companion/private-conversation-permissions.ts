import { conversationPermissionsSchema } from "../../modules/remote/private-conversation-contracts.js";
import { randomUUID } from "node:crypto";
import { z } from "zod";
import { id } from "../../modules/contracts/index.js";
import {
  PrivateConversationConsent,
  PrivateConversationConsentError,
} from "../../modules/remote/private-conversation-consent.js";
import {
...
type Prepared = Awaited<ReturnType<PrivateConversationConsent["prepare"]>>;
type Review = {
  id: string;
  createdAt: number;
  expiresAt: number;
...
export class CompanionConversationPermissions {
  ...
  async prepare(raw: unknown) {
    ...
    try {
      return await this.live().withVerifiedDevice(async (scope) => {
        ...
        if (!binding) throw new PrivateConversationConsentError("DENIED");
