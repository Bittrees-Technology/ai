...
Migration `004-controls.sql` and `RemoteCommandStore` add a durable pause/cancel queue. Existing devices receive `controls_enabled=false`; this package cannot enable them. A future separately consented command-delivery scope, protected routes and Mac executor must be implemented before exposing controls. Status-publication credentials are not silently upgraded.
