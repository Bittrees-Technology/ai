...
These are module APIs, not an in-app backup or restore flow. A task snapshot alone does not include memory. The two snapshot calls are not a coordinated atomic pair; a complete recovery workflow must quiesce writes, capture both stores consistently and verify key recovery. Do not treat JSON export as an importable backup or the existing native app as providing a backup button. Full native backup/restore and installation/rollback acceptance remain open.
