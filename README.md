...
Tasks with failed, cancelled or expired prerequisites now settle as failed during local worker polling, in batches of at most128 per tick. Deeper chains settle on subsequent ticks; transient retries and paused prerequisites stay pending. Encrypted failure details and task/outbox history explain the outcome without running the dependent task. Later independent conversation work can then proceed. See [dependency failure handling](docs/task-dependency-failures.md). Configurable queue capacity remains pending. Messages, read/delivery/acknowledgement receipts and due/closed/overdue check-ins are persisted. Messages and tasks share conversation sequencing; receipts do not complete tasks. Personal-device inboxes may name an agent or manager but cannot add another user. No source-app side effects are attempted or retried by this foundation.
...
An internal [memory-candidate contract and Mac model probe](docs/memory-candidate-evaluation.md) now records bounded, source-snapshot-bound suggestions with exact excerpt checks. Suggestions remain model-generated, unapproved and unverified; excerpt presence does not prove a paraphrase. Real synthetic runs compare the small model, original 9B and Huihui 9B across two prompt versions. Explicit queue, review and selected-candidate saving are integrated for local and source-backed tasks. Source-backed preparation shows the current request/result; generation, review and saving recheck source access. Broader quality acceptance remains open; no automatic extraction rule is enabled.
...
`apps/remote-web` contains the standalone web screen's source. Run `npm run build:remote` (also included in `npm run build`) and pass the absolute `dist/remote-web` directory as the remote app factory's optional `assets` setting. The service serves only the index, entry script, local favicon, bounded hashed JavaScript/CSS assets and public origin/chain settings. Static responses retain no-store, direct TLS/Host checks and a self-only script/style/image/connect policy; no external assets or analytics are used. Without `assets`, the existing API-only behavior remains.

The signed-in screen explicitly opens reviewed browser registration and recovery controls. Retained keys and lifecycle records use IndexedDB; cross-tab authentication uses Web Locks and content-free local-storage revision/acknowledgment records. Failed cookie cleanup persists across reload. Recovery controls close when authenticated scope is lost, and status rendering cannot enable unacknowledged recovery actions. Saved code, encrypted backup verification and activation remain separate from Mac pairing and private-task permission. See [production browser recovery](docs/production-browser-recovery.md) for build, storage, CSP, acceptance and deployment limits.
...
The [durable private-task sender](docs/private-task-outbox.md) preserves original ciphertext, deduplicates local enqueue retries, fences handoffs with current per-peer consent and verifies encrypted destination acceptance. Schema15 includes encrypted owner-scoped outbox export/deletion and locks restored entries. No live sending route or installed-app update is enabled; key persistence, reciprocal UI, receipt production, relay transport and independent review remain open.
...
[Peer-key possession checks](docs/private-peer-key-checks.md) now require each endpoint to finish its own short-lived encrypted challenge before task permission can be approved or used. Exact ciphertext retries and proof history survive restart; schema19 preserves tasks while requiring fresh review of older grants. Authenticated Mac routes are available behind the existing development gate; exchange UI/browser keys/transport and independent acceptance remain open.
...
Encrypted AutoNote approval preparation and explicit packet delivery are described in [companion approval controls](docs/autonote-approval-host.md). They require the disabled-by-default `BITTREES_PRIVATE_AUTONOTE_APPROVAL=1` setting; browser approval and source-save integration remain unfinished.
