...
const actor = authoritySchema.parse({
  userId: "alice",
  subjectId: "alice",
  tenantId: "workspace",
  deviceId: "personal",
...
test("authority is the intersection of all four independently verified scopes", () => {
  ...
  for (const key of ["source", "delegation", "connector", "policy"] as const) {
    for (const patch of [
      ...
      Object.assign(changed[key], patch);
      assert.equal(
        isAuthorized(actor, action, changed, 100),
        false,
...
test("strict request schema rejects caller authority and unbounded input", () => {
  ...
  assert.equal(
    requestSchema.safeParse({ ...body, authority: actor }).success,
    false,
  );
  assert.equal(
    requestSchema.safeParse({ ...body, prompt: "x".repeat(32001) }).success,
    false,
  );
});
test("remote status cannot carry prompts, titles, subjects or arbitrary errors", () => {
  ...
  for (const key of [
    ...
    assert.equal(
      remoteStatusSchema.safeParse({ ...base, [key]: "PRIVATE" }).success,
      false,
    );
  assert.equal(
    remoteStatusSchema.safeParse({ ...base, errorCode: "PRIVATE" }).success,
    false,
  );
});
