...
import {
  ...
  privateResumeGrantSchema,
} from "./private-resume-consent.js";
...
const valueSchema = z
  .strictObject({
    ...
    grant: privateResumeGrantSchema,
  ...
  .refine(
    (v) =>
      v.offer.permissionId === v.grant.id &&
      v.offer.taskId === v.grant.choices.taskId &&
      v.offer.taskRevision === v.grant.choices.taskRevision &&
      v.offer.modelDigest === v.grant.choices.modelDigest &&
...
type Handle = Awaited<ReturnType<PrivateResumeConsent["resolve"]>>;
export class ResumeOfferError extends Error {
  constructor(
...
const purpose = (owner: Owner, id: string) =>
  JSON.stringify(["private-resume-offer:v1", owner.tenantId, owner.userId, id]);
function read(store: Store, vault: Vault, owner: Owner, id: string): Entry {
  try {
    const r = store.db
      .prepare(
        "SELECT revision,locked,payload FROM private_resume_offers WHERE user_id=? AND tenant_id=? AND id=?",
      )
      .get(owner.userId, owner.tenantId, id) as
      { revision: number; locked: number; payload: Buffer } | undefined;
    if (!r) throw new ResumeOfferError("DENIED");
    if (
    ...
    if (e instanceof ResumeOfferError) throw e;
    throw new ResumeOfferError("STORAGE_UNAVAILABLE");
  }
}
export function exportPrivateResumeOffers(
  store: Store,
  vault: Vault,
  owner: Owner,
) {
  const rows = store.db
    .prepare(
      "SELECT id FROM private_resume_offers WHERE user_id=? AND tenant_id=? ORDER BY id LIMIT 257",
    )
    .all(owner.userId, owner.tenantId) as { id: string }[];
  if (rows.length > 256) throw new ResumeOfferError("CAPACITY");
...
export class PrivateResumeOffers {
  private inflight = 0;
  constructor(
    private store: Store,
    private vault: Vault,
  ...
  private input<T>(schema: z.ZodType<T>, raw: unknown): T {
    ...
    if (!p.success) throw new ResumeOfferError("DENIED");
    return p.data;
  }
  private hash(kind: string, raw: unknown) {
    return this.vault.fingerprint([
      "private-resume-offer:v1",
      this.owner,
      kind,
      raw,
    ]);
  ...
  private async bounded<T>(fn: () => Promise<T>) {
    if (this.inflight >= 4) throw new ResumeOfferError("CAPACITY");
    this.inflight++;
    try {
      return await fn();
    } catch (e) {
  ...
  private checked(e: Entry, h: Handle) {
    h.check();
    const p = h;
    const task = this.store.get(this.owner, p.grant.choices.taskId);
    const permission = this.store.remoteResumes
      .history(this.owner)
      .find((v) => v.id === p.grant.id)?.permission;
    if (
      task.status !== "paused" ||
      task.revision !== p.grant.choices.taskRevision ||
      !permission ||
      permission.consumed
    )
      throw new ResumeOfferError("DENIED");
  ...
  private sealed(e: Entry) {
    ...
    if (payload.length > 16384) throw new ResumeOfferError("CAPACITY");
    return payload;
  }
  private save(e: Entry) {
    if (e.revision >= Number.MAX_SAFE_INTEGER)
      throw new ResumeOfferError("CAPACITY");
    const updated = this.store.db
      .prepare(
        "UPDATE private_resume_offers SET revision=revision+1,payload=? WHERE user_id=? AND tenant_id=? AND id=? AND revision=?",
      )
      .run(
        this.sealed(e),
        this.owner.userId,
    ...
    if (updated.changes !== 1) throw new ResumeOfferError("CONFLICT");
    e.revision++;
  }
  get(raw: unknown) {
    ...
    return this.bounded(async () => {
      ...
      return this.store.db
        .transaction(() => {
          handle.check();
          const p = handle;
          if (!p) throw new ResumeOfferError("DENIED");
          const existing = this.store.db
            .prepare(
              "SELECT id FROM private_resume_offers WHERE user_id=? AND tenant_id=? AND client_hash=?",
            )
          ...
          const count = this.store.db
            .prepare(
              "SELECT COUNT(*) AS count FROM private_resume_offers WHERE user_id=? AND tenant_id=?",
            )
            .get(this.owner.userId, this.owner.tenantId) as { count: number };
          if (count.count >= 256) throw new ResumeOfferError("CAPACITY");
          const id = randomUUID(),
            issuedAt = this.now(),
            grant = p.grant,
            offer = privateResumeOfferSchema.parse({
              version: 1,
              type: "task.resume.offer",
              permissionId: grant.id,
              taskId: grant.choices.taskId,
              taskRevision: grant.choices.taskRevision,
              modelDigest: grant.choices.modelDigest,
              issuedAt,
          ...
          this.store.db
            .prepare(
              "INSERT INTO private_resume_offers(user_id,tenant_id,id,client_hash,revision,locked,payload) VALUES(?,?,?,?,1,0,?)",
            )
            .run(
              this.owner.userId,
              this.owner.tenantId,
