...
/** Separate source consent and secret. This connector does not yet dispatch saves. */
  ...
  private async exchange(code: string, verifier: string) {
    try {
      const response = await this.transport(
        origin + "/api/integrations/ai/approval-exchange",
        {
          method: "POST",
          redirect: "error",
          credentials: "omit",
      ...
      try {
        for (;;) {
          ...
          if (done) break;
          size += value.length;
          if (size > 8192) throw new ConnectorError("INVALID_SOURCE");
          chunks.push(value);
        }
      } finally {
        await reader.cancel();
  ...
  async finish(id: string, code: string) {
    return this.exclusive(async () => {
      ...
      if (JSON.stringify(parent) !== pending.parent)
        throw new ConnectorError("SOURCE_DENIED");
      pending.check();
      const grant = await this.exchange(code, pending.verifier);
      pending.check();
      if (
