/** Actual compiled task24 -> current task26; disposable data, no Keychain or network. */
...
if (!process.argv[2])
  throw Error(
    "Provide the verified compiled task24 engine directory after building current source.",
  );
...
assert.equal(
  legacyHash,
  "f608e692db4c90213a6d238a478358fadab67f6ecdd6379b882ccb5b0913f982",
  "Use the verified task24 compiled store",
);
...
try {
  ...
  assert.equal(old.db.pragma("user_version", { simple: true }), 24);
  ...
  assert.equal(
    old.db
      .prepare(
        "SELECT count(*) n FROM sqlite_master WHERE name='private_response_delivery'",
      )
      .get().n,
    0,
  );
  ...
  assert.equal(current.db.pragma("user_version", { simple: true }), 36);
  ...
  assert.equal(rollback.db.pragma("user_version", { simple: true }), 24);
  ...
  const result = {
    ...
    checks: [
      ...
      "empty delivery journal grants no authority",
  ...
  await writeFile(
    join(
      repo,
      "docs/evidence/private-response-delivery-schema-compatibility-2026-09-24.json",
    ),
